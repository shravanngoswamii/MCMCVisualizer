...
async function main(): Promise<void> {
	...
	const afterSlice = flags["warmup"]
		? data.slice(parseInt(flags["warmup"] as string, 10))
		: data;
	const afterVars = flags["vars"]
		? afterSlice.filterVariables(
				(flags["vars"] as string).split(",").map((s) => s.trim()),
			)
		: afterSlice;
	const filtered = flags["chains"]
		? afterVars.filterChains(
				(flags["chains"] as string).split(",").map((s) => s.trim()),
			)
		: afterVars;
...
function dispatch(
	...
	switch (cmd) {
		case "summary":
			return runSummary(data, fmt);
		case "diagnose":
			return runDiagnose(data, fmt);
		case "rhat":
			return runRhat(data, fmt);
		case "ess":
			return runEss(data, fmt);
		case "convert":
			return runConvert(data, (flags["to"] as string) ?? "json");
		case "plot":
			return runPlot(
				data,
				(flags["type"] as string) ?? "trace",
				(flags["var"] as string) ?? data.variableNames[0] ?? "",
				flags,
			);
		default:
			die(`Unknown command: "${cmd}". Run "mcmc help" for usage.`);
	}
}
...
function runRhat(data: InferenceData, fmt: string): string {
	const rows = data.summary().map((r) => ({
		variable: r.variable,
		rhat: r.rhat,
		splitRhat: r.splitRhat,
		status: rhatStatus(r.rhat),
	}));
	if (fmt === "json") return JSON.stringify(rows, null, 2);
...
function runEss(data: InferenceData, fmt: string): string {
	const rows = data.summary().map((r) => ({
		...
		essBulk: r.bulkEss,
		essTail: r.tailEss,
		essPerDraw: r.essPerDraw,
		count: r.count,
	}));
	if (fmt === "json") return JSON.stringify(rows, null, 2);
	return renderTable(
		["variable", "essBulk", "essTail", "essPerDraw", "count"],
		rows,
	);
}
...
function runPlot(
	...
	if (typeof specFn !== "function") {
		const available = Object.keys(plots)
			.filter((k) => k.endsWith("PlotSpec"))
			.map((k) => k.replace("PlotSpec", ""))
			.join(", ");
		die(`Unknown plot type: "${type}". Available: ${available}`);
	}
	const opts = {
		theme: (flags["theme"] as string) ?? "dark",
		height: flags["height"] ? +flags["height"] : undefined,
		width: flags["width"] ? +flags["width"] : undefined,
	};
	return JSON.stringify(
		(specFn as (d: InferenceData, v: string, o: unknown) => unknown)(
			data,
			variable,
			opts,
		),
...
function loadData(text: string, fmt?: string): InferenceData {
	switch (fmt) {
		...
		case "arviz-json":
			return new MCMCData(parseArviZJSONPosterior(text));
		default:
			return fromAutoDetect(text);
	}
}
...
function renderSummaryTable(rows: VariableSummary[]): string {
	...
	return renderRawTable(
		cols.map((c) => c[1]),
		rows.map((r) => cols.map(([f, , fn]) => fn(r[f]))),
	);
}
...
function renderTable(cols: string[], rows: Record<string, unknown>[]): string {
	...
	return renderRawTable(
		cols,
		rows.map((r) => cols.map((c) => fmtCell(r[c]))),
	);
}
...
function renderRawTable(headers: string[], rows: string[][]): string {
	...
	const body = rows
		.map((r) => r.map((c, i) => c.padStart(widths[i]!)).join(sep))
		.join("\n");
	return `\n ${head}\n ${line}\n${body
		.split("\n")
		.map((l) => " " + l)
...
function fmt1(v: number): string {
	return isNaN(v) ? "NaN" : v.toFixed(1);
}
function fmt4(v: number | undefined): string {
	return v === undefined || isNaN(v) ? "NaN" : v.toFixed(4);
}
function fmt6(v: number): string {
	return isNaN(v) ? "NaN" : v.toFixed(6);
}
...
function rhatStatus(v: number | undefined): string {
	...
	return v > 1.1 ? "FAIL" : v > 1.01 ? "WARN" : "OK";
}
