 ...
 */
export function parseArviZJSON(
	input: string | object,
): Map<string, Map<string, ChainData>> {
	const json =
		typeof input === "string"
			? (JSON.parse(input) as ArviZJSON)
			: (input as ArviZJSON);
 ...
 */
export function parseArviZJSONPosterior(
	input: string | object,
...
function _groupToChains(
	dataVars: Record<string, ArviZVariable>,
): Map<string, ChainData> {
	...
	for (let ci = 0; ci < nChains; ci++) {
		...
		for (const idx of indices) {
			...
			for (let di = 0; di < nDraws; di++) {
				let node: unknown = chainData[di],
					valid = true;
				for (const i of idx) {
					if (!Array.isArray(node) || i >= (node as unknown[]).length) {
						valid = false;
						break;
					}
