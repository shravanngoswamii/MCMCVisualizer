...
export function summaryTable(
	...
	function render() {
		...
		for (const s of summaries) {
				...
				s.rhat === undefined
					...
					: s.rhat < 1.05
						...
						: s.rhat < 1.1
							? "#eab308"
							: "#ef4444";
			const splitRhatColor =
				s.splitRhat === undefined
					? mutedColor
					: s.splitRhat < 1.05
						? "#22c55e"
						: s.splitRhat < 1.1
							...
							: "#ef4444";
			const essColor =
				s.ess > 400 ? "#22c55e" : s.ess > 100 ? "#eab308" : "#ef4444";
			const bulkEssColor =
				s.bulkEss > 400 ? "#22c55e" : s.bulkEss > 100 ? "#eab308" : "#ef4444";
			const tailEssColor =
				s.tailEss > 400 ? "#22c55e" : s.tailEss > 100 ? "#eab308" : "#ef4444";
			const gewekeColor = isNaN(s.geweke.z)
				? mutedColor
				: Math.abs(s.geweke.z) < 1.96
					? "#22c55e"
					: Math.abs(s.geweke.z) < 2.58
						? "#eab308"
						: "#ef4444";
			...
			html += `<td style="padding:8px 14px;color:${splitRhatColor};font-variant-numeric:tabular-nums">${s.splitRhat !== undefined ? s.splitRhat.toFixed(3) : "\u2014"}</td>`;
