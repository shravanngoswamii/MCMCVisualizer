...
export function diagnosticsHeatmapPlot(
	...
	function render() {
		...
		const metrics = [
			{
				...
				score: (summary: VariableSummary) =>
					scoreUpper(summary.rhat, 1.01, 1.1),
			},
			{
				label: "Split R-hat",
				...
				score: (summary: VariableSummary) =>
					scoreUpper(summary.splitRhat, 1.01, 1.1),
			},
			{
				label: "ESS / draw",
				...
				score: (summary: VariableSummary) =>
					scoreLower(summary.essPerDraw, 0.25, 0.05),
			},
			{
				label: "Bulk ESS",
				...
				score: (summary: VariableSummary) =>
					scoreLower(summary.bulkEss, 400, 100),
			},
			{
				label: "Tail ESS",
				...
				score: (summary: VariableSummary) =>
					scoreLower(summary.tailEss, 400, 100),
			},
			{
				label: "MCSE / sd",
				raw: (summary: VariableSummary) =>
					summary.stdev === 0 ? NaN : summary.mcse / summary.stdev,
				text: (summary: VariableSummary) =>
					summary.stdev === 0
						? "0.000"
						: formatValue(summary.mcse / summary.stdev),
				score: (summary: VariableSummary) =>
					scoreUpper(
			...
			},
			{
				...
				text: (summary: VariableSummary) =>
					formatValue(Math.abs(summary.geweke.z)),
				score: (summary: VariableSummary) =>
					scoreUpper(Math.abs(summary.geweke.z), 1.96, 3.0),
			},
		];

		const x = metrics.map((metric) => metric.label);
		const y = summaries.map((summary) => summary.variable);
		const z = summaries.map((summary) =>
			metrics.map((metric) => metric.score(summary)),
		);
		const text = summaries.map((summary) =>
			metrics.map((metric) => metric.text(summary)),
		);
		const customdata = summaries.map((summary) =>
		...
		const trace = {
			...
			hovertemplate:
				"%{y}<br>%{x}: %{text}<br>Risk score=%{z:.2f}<extra></extra>",
		};
